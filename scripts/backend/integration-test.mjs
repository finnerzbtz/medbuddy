...
const env = await readFile(new URL('.env.backend.development.local', root), 'utf8');
const connectionString = env
  .split('\n')
  .find((x) => x.startsWith('DATABASE_URL='))
  ?.slice(13);
if (!connectionString || !connectionString.includes('ep-fancy-morning-za5k9op1'))
  throw new Error('Tests require the isolated development branch.');
const pool = new pg.Pool({ connectionString, max: 2 });
const auth = 'https://ep-fancy-morning-za5k9op1.neonauth.c-2.eu-west-2.aws.neon.tech/reminduh/auth';
const api =
  'https://ep-fancy-morning-za5k9op1.apirest.c-2.eu-west-2.aws.neon.tech/reminduh/rest/v1';
const origin = 'http://127.0.0.1:5177';
const out = new URL('rebuild/generated/qa-cloud/', root);
